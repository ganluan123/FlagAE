...
#'
#' @description Both functions \code{gci} and \code{gci2} plot the top AEs with
...
gci <- function(aedata, ptnum=10, conf.level=0.95) {
  ...
  # start the plotting
  plot(NA, xlim=c(-1,1), ylim=c(-5,5*ptnum), main=paste0("Confidence interval of top ", ptnum," TEAE in difference by treatment group"),
